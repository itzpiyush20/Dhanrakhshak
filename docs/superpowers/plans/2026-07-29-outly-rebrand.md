...
**Goal:** Rename the product from `Dhanrakshak` to `Outly: Expense & Budget Tracker` across every code, config, storage, and documentation surface, without touching the third-party `Intrak` analytics integration or any Razorpay wire-protocol field name — both of which look similar but are unrelated systems that must not be renamed.
...
Every task below touches strings containing `dhanrakshak`. Two other strings exist in this codebase that look similar but are **NOT to be renamed under any circumstances**:
...
If a task's file also contains `intrak` or `razorpay_*` text, edit only the `dhanrakshak`/`Dhanrakshak`/`DHANRAKSHAK` occurrences and leave everything else on that line/file untouched. Each task below explicitly calls this out again wherever it applies.
...
```json
  ...
  "name": "Dhanrakshak — Personal Finance Tracker",
...
Run: `grep -ric dhanrakshak package.json capacitor.config.ts public/manifest.json public/sw.js`
...
Note: this function's job is to delete a *stale legacy key from a prior app version*. Renaming the literal here just means it now purges the Outly-era stale key instead of the Dhanrakshak-era one — this is correct behavior, not a bug, since the actual pre-v2 key it originally targeted is long gone from any current user's storage anyway.
...
```ts
localStorage.getItem('dhanrakshak_merchant_weights')
```
```ts
localStorage.getItem('dhanrakshak_merchant_settings')
```
```ts
...
Replace every `dhanrakshak_merchant_weights` → `outly_merchant_weights` and every `dhanrakshak_merchant_settings` → `outly_merchant_settings` in this file (6 occurrences total across get/set call sites).
...
Find the two occurrences of `dhanrakshak_tester_feedback` (a get and a set call) and replace both with `outly_tester_feedback`.
...
Run: `grep -rn "dhanrakshak_" src/services/googleAuth.ts src/services/emailScanner.ts src/services/learningEngine.ts src/services/feedback.ts`
Expected: no output.
...
- [ ] **Step 1: Rename all `dhanrakshak_`-prefixed key literals in this file**

Replace every occurrence of each of these string prefixes (they appear both as plain literals and inside template literals like `` `dhanrakshak_sub_status_${state.user.id}` ``):

| Old prefix | New prefix |
| --- | --- |
| `dhanrakshak_sub_status_` | `outly_sub_status_` |
| `dhanrakshak_sub_expires_` | `outly_sub_expires_` |
| `dhanrakshak_sub_plan_` | `outly_sub_plan_` |
| `dhanrakshak_promo_code_` | `outly_promo_code_` |
| `dhanrakshak_active_financial_year_` | `outly_active_financial_year_` |
| `dhanrakshak_daily_scan_time_` | `outly_daily_scan_time_` |
| `dhanrakshak_device_id` | `outly_device_id` |
| `dhanrakshak_requesting_gmail_scope` | `outly_requesting_gmail_scope` |

Every one of these is used consistently by prefix — e.g. `` `dhanrakshak_sub_status_${state.user.id}` `` — so a straightforward string substitution of each left-hand value with its right-hand value, everywhere it appears in this file, is correct and complete.
...
Run: `grep -n -i "dhanrakshak" src/context/AuthContext.tsx`
Expected: no output.
...
| Old prefix | New prefix |
| --- | --- |
| `dhanrakshak_checklist_dismissed_` | `outly_checklist_dismissed_` |
| `dhanrakshak_last_seen_month_` | `outly_last_seen_month_` |
| `dhanrakshak_visited_analytics_` | `outly_visited_analytics_` |
| `dhanrakshak_dashboard_widgets` | `outly_dashboard_widgets` |
| `dhanrakshak_ls_migration_done` | `outly_ls_migration_done` |
...
| Old | New |
| --- | --- |
| `dhanrakshak_analytics_advanced` | `outly_analytics_advanced` |
| `dhanrakshak_visited_analytics_` | `outly_visited_analytics_` |
...
| Old | New |
| --- | --- |
| `dhanrakshak_theme` | `outly_theme` |
| `dhanrakshak_theme_changed` | `outly_theme_changed` |
...
```ts
dhanrakshak_ignored_subscriptions_
```
to:
```ts
...
Run: `grep -n -i "dhanrakshak" src/pages/DashboardPage.tsx src/pages/AnalyticsPage.tsx src/pages/SettingsPage.tsx src/pages/SubscriptionsPage.tsx`
Expected: no output.
...
| `dhanrakshak_theme` | `outly_theme` |
| `dhanrakshak_theme_changed` | `outly_theme_changed` |
| `dhanrakshak_dismissed_notifications` | `outly_dismissed_notifications` |
| `dhanrakshak_notifications_cache` | `outly_notifications_cache` |
| `dhanrakshak_security_acknowledged` | `outly_security_acknowledged` |
| `dhanrakshak_pwa_dismissed` | `outly_pwa_dismissed` |
...
Rename `dhanrakshak_last_auto_reload` → `outly_last_auto_reload` (appears as a sessionStorage key literal).

- [ ] **Step 4: `src/components/CookieConsent.tsx` — rename storage key**

Rename `dhanrakshak_cookie_consent` → `outly_cookie_consent` (two occurrences: a get and a set).

- [ ] **Step 5: `src/components/InstallPrompt.tsx` — rename storage key and copy**

Rename `dhanrakshak_install_prompt_dismissed` → `outly_install_prompt_dismissed`.
...
Rename `dhanrakshak_theme` → `outly_theme` (one occurrence).

- [ ] **Step 8: `src/main.tsx` — rename storage key**

Rename `dhanrakshak_oauth_provider_token` → `outly_oauth_provider_token` (one occurrence — this mirrors the same key renamed in `googleAuth.ts` Task 2 Step 1; both must match exactly since this file also purges that legacy key on boot).
...
```html
    <meta name="description" content="Dhanrakshak — Your personal wealth guardian. Track expenses, manage budgets, and gain financial clarity." />
```
→
```html
...
```html
    <meta property="og:url" content="https://dhanrakshak-five.vercel.app/" />
    <meta property="og:title" content="Dhanrakshak — Personal Finance Dashboard" />
    ...
    <meta property="og:image" content="https://dhanrakshak-five.vercel.app/favicon.svg" />
```
→
```html
...
```html
    <meta property="twitter:url" content="https://dhanrakshak-five.vercel.app/" />
    <meta property="twitter:title" content="Dhanrakshak — Personal Finance Dashboard" />
    ...
    <meta property="twitter:image" content="https://dhanrakshak-five.vercel.app/favicon.svg" />
```
→
```html
...
```html
    <title>Dhanrakshak — Personal Finance Dashboard</title>
```
→
```html
...
```js
          var stored = localStorage.getItem('dhanrakshak_theme');
```
→
```js
...
```html
    <meta name="apple-mobile-web-app-title" content="Dhanrakshak" />
```
→
```html
...
```js
              var lastReload = sessionStorage.getItem('dhanrakshak_last_auto_reload');
```
→
```js
...
(There is a second `dhanrakshak_last_auto_reload` reference further down in the same script block for the `setItem` call — rename that one too.)

Rename the purge-version key:

```js
dhanrakshak_purge_v4
```
→
```js
outly_purge_v1
...
Run: `grep -n -i "dhanrakshak" src/layouts/AppLayout.tsx src/layouts/MarketingLayout.tsx src/components/AutoUpdateChecker.tsx src/components/CookieConsent.tsx src/components/InstallPrompt.tsx src/components/ErrorBoundary.tsx src/App.tsx src/main.tsx index.html`
Expected: no output.
...
Replace every occurrence of `Dhanrakshak` with `Outly` in this file. Specific lines to change:
...
There is also an uppercase `DHANRAKSHAK` literal elsewhere in this file (a promo code or banner string) — find it with `grep -n "DHANRAKSHAK" src/pages/PricingPage.tsx` and replace it with `OUTLY`.
...
Replace every occurrence of `Dhanrakshak` with `Outly` in this file (9 occurrences: intro paragraph, service description, email-access clause, subscription clause, cancellation clause, liability clause). Do not change wording, refund terms, or the structure of any clause — only the product name token changes. For example:
...
Apply the same literal `Dhanrakshak` → `Outly` substitution to the remaining 8 occurrences in this file, changing nothing else on each line.
...
Replace every occurrence of `Dhanrakshak` with `Outly` (5 occurrences). For example:
...
Run: `grep -n -i "dhanrakshak" src/pages/LandingPage.tsx src/pages/AboutPage.tsx src/pages/PricingPage.tsx src/pages/TermsPage.tsx src/pages/PrivacyPage.tsx src/pages/RefundPage.tsx src/pages/SupportPage.tsx`
Expected: no output.
...
Note: `SUPPORT_EMAIL` is deliberately left as `support@dhanrakshak.in` — per the approved spec, no new domain is being purchased this phase, so this address stays live. Do not change it.
...
```css
   Dhanrakshak — Design Token System v3
```
→
```css
...
Run: `grep -rn -i "dhanrakshak" src/pages/ExpensesPage.tsx src/pages/BudgetsPage.tsx src/pages/PendingPage.tsx src/pages/ProfilePage.tsx src/pages/ForgotPasswordPage.tsx src/pages/ResetPasswordPage.tsx src/constants/index.ts src/types/index.ts src/index.css`
Expected: no output.
...
This file has two occurrences of `https://dhanrakshak-five.vercel.app` — one in a `process.env.ALLOWED_ORIGIN = ...` test setup line, one in a request `origin` header. Replace both with `https://outly.vercel.app`. Do not touch the `intrak_*` note fields or `razorpay_*` fields elsewhere in this test file.

- [ ] **Step 7: `api/save-google-refresh-token.test.ts` — rename origin fallback in all four occurrences**

Replace all four occurrences of `https://dhanrakshak-five.vercel.app` with `https://outly.vercel.app` in this file (test env setup and request `origin` headers across multiple test cases).
...
This also fixes a pre-existing issue: the old fallback pointed at `digest@dhanrakshak.app`, a domain never verified in Resend, so unconfigured deployments were already silently failing to send. `onboarding@resend.dev` is Resend's shared sender that works without domain verification.
...
Run: `grep -rn "dhanrakshak" api/create-order.ts api/verify-payment.ts api/gemini-proxy.ts api/refresh-google-token.ts api/save-google-refresh-token.ts api/create-order.test.ts api/save-google-refresh-token.test.ts api/weekly-digest.ts`
Expected: no output.
...
At the top of the file, directly under the `# Dhanrakshak — Personal Finance Dashboard` heading, add a note (before renaming the heading) recording the prior name. Change:
...
→
```md
- Can view all feedback and signin logs via the `%@dhanrakshak.in` RLS backdoor (still points at the support domain; update if that changes — see Task 8 Step 8 note on `SUPPORT_EMAIL` for why this wasn't renamed this phase)
```

- [ ] **Step 2: `PRODUCT.md` and `DESIGN.md` — rename product references**

In each file, replace `Dhanrakshak` with `Outly` in all prose headings/body text. In `DESIGN.md`, also update the `dhanrakshak_theme` key reference to `outly_theme` (this is documentation of the actual key renamed in Task 4/5, so it must match).
...
Replace every occurrence of `Dhanrakshak` with `Outly`, and every occurrence of `dhanrakshak-five.vercel.app` with `outly.vercel.app` in this file (7 and 5 occurrences respectively). This file is what the owner will use to submit for Google verification after this rebrand lands, so it must be fully consistent with the new brand before that submission happens.

- [ ] **Step 6: `user_login_guide.md` — rename product references**

Replace every occurrence of `Dhanrakshak` with `Outly` (9 occurrences).
...
Run: `grep -rn -i "dhanrakshak" README.md PRODUCT.md DESIGN.md MOBILE_SETUP.md TRANSFER_GUIDE.md GOOGLE_VERIFICATION_GUIDE.md user_login_guide.md supabase/schema.sql`
Expected: only the intentional `> Formerly named Dhanrakshak.` line in `README.md` and the `%@dhanrakshak.in` domain reference (both deliberately preserved per the steps above).
...
- `README.md` — the `> Formerly named Dhanrakshak.` note and the `%@dhanrakshak.in` RLS reference
- `src/constants/index.ts` — `SUPPORT_EMAIL: 'support@dhanrakshak.in'`
...
- No visible `Dhanrakshak` text anywhere in the rendered UI
...
```bash
git add -A
git commit -m "rebrand: fix remaining stray Dhanrakshak references found in final sweep"
```
...
8. **Cleanup** (only after confirming production is healthy on the new domain): remove the old `dhanrakshak-five.vercel.app` entries from Google Cloud origins and Supabase Redirect URLs.
